...
export class TrioBettingClass extends BettingClass implements BettingInterface {
  amount: number;
  betType: BetTypeEnum;
  ...
  generateResult(
    ...
    if (roundResult === this.betType) {
      if (Object.keys(chipsBetsTotal).length > 0) {
        ...
        const freePool = this.getFreePool(playerEarned, totalChipsDeal);
        result.freePool = freePool;

        const totalPrizePool = Number(totalChipsDeal) + Number(currentFreePool);
