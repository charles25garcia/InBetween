...
const chipsWithdrawalHistory = ref<WithdrawalDto[]>([]);

const requestChipsInput = useInputField({
  id: "chipsRequest",
  displayName: "Amount",
  type: "number",
  required: true,
...
async function submitWithdrawalRequest() {
  try {
    ...
    const res = await WithdrawalHttp.sendChipsWithdrawalRequest(
      profileStore.userProfile.id,
      +requestChipsInput.getValue()
    );

    updateChips(res.chips);

    await loadHistory();

    requestChipsInput.setValue("");
...
function showConfirmation() {
  if (requestChipsInput.getValue().length < 1) {
    return;
  }

  const requestedAmount = Number(requestChipsInput.getValue());

  if (Number(profileStore.userProfile.userStats.chips) < requestedAmount) {
    errorMessage.value = {
...
async function loadHistory() {
  isLoading.value = true;
  const res = await WithdrawalHttp.getChipsHistory(profileStore.userProfile.id);
  await profileStore.fetchUserProfile();

  chipsWithdrawalHistory.value = res.sort(
    (a, b) =>
...
function getTableData() {
  return chipsWithdrawalHistory.value.map(
    ({
      id,
      amount,
      status,
...
async function cancelWithdrawalRequest() {
  ...
  try {
    const res = await WithdrawalHttp.cancelChipsRequest(
    ...
    console.log("Chips withdrawal cancelation request error: " + e);
  }
...
function updateChips(chips: number) {
...
<template>
  <common-feature-container title="Chips" :description="`Total: ${profileStore.userProfile?.userStats?.chips || 0}`">
    <div class="chips-withdrawals-container">
      ...
      <br />
      <common-table
        export-file-name="Chips Withdrawal History"
        :items="getTableData()"
